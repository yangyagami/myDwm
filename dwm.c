...
static void killallclients(const Arg *arg);
...
static void overview_grid(Monitor *);
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w - 2 * bargap, bh, lrpad / 2, m->sel->name, 0);
			if (m->sel->isfloating)
				drw_rect(drw, x + boxs, boxs, boxw, boxw, m->sel->isfixed, 0);
		} else {
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_rect(drw, x, 0, w - 2 * bargap, bh, 1, 1);
		}
	}
...
overview_grid(Monitor *m) {
	...
	ch = (m->wh - 2 * gappx - (row - 1) * gappx) / row;
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
		if (current_col != 0 && current_col % max_col == 0) {
			cx = gappx;
			current_col = 0;
			current_row++;
		}

		cx = current_col * cw + (current_col + 1) * gappx;
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating)
		resize(selmon->sel, selmon->mw / 2 - 400, selmon->mh / 2 - 300,
			800, 600, 0);
	arrange(selmon);
}
